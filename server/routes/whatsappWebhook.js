...
const { handleMessage } = await import("../agents/orchestrator.js");
...
if (/^(what('s| is) your name|who are you|what are you|איך קוראים לך|מה השם שלך|מה שמך|מי את|מי אתה)[?.!]?\s*$/i.test(body.trim())) {
    ...
    console.log(`🤖 [WhatsApp] Pipeline complete — tool: ${result.tool}, success: ${result.success}, model: ${targetModel || 'default'}`);
